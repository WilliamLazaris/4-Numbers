...
def factorial_or_sqrt(n):
    ...
    if n >= 0 and n == int(n):  # Factorial is defined for non-negative integers
        # Limit the factorial calculation to prevent overly large results
        if n <= 100:  # Adjust this limit based on acceptable sizes
                ...
                for c_expr, c in factorial_or_sqrt(numbers[2]):
                    for d_expr, d in factorial_or_sqrt(numbers[3]):
                        try:
                            ...
                            continue  # Skip any combination that causes overflow
    return combinations
...
def main():
    try:
        ...
        if combinations:
            print(f"The possible combinations of {int(A)}, {int(B)}, {int(C)}, and {int(D)} (including factorials and square roots) that add, subtract, multiply, divide, or exponentiate to equal 10 are:")
            for combination in combinations:
                print(combination)
        else:
            print(f"There are no combinations of {int(A)}, {int(B)}, {int(C)}, and {int(D)} (including factorials and square roots) that add, subtract, multiply, divide, or exponentiate to equal 10.")
    except ValueError:
